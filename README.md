...
有的开发者选择传输“遥测”数据，其中包括设备标识和照片中的粗略地理位置。而灵动照片不以反作弊或免费额度为理由收集 IDFV，也不设置每日使用次数：提取颜色不需要云端，生成卡片不需要设备追踪，完全免费更不需要每日配额。

我们更愿意把选择权交还给用户。自 Build 1050 起，设置中加入了“显示应用标题”选项：即使不会自行编译，也可以隐藏编辑界面左上角的“灵动照片”，且不影响导出作品。工具装在你的设备上，应当服务于你的创作，而不是把你变成它的数据或广告位。
